...
public class FileUtil {
    ...
    private static boolean isFromJar() {
        boolean blReturn = false;
        String name = "" + FileUtil.class.getResource("FileUtil.class");
    ...
    public static String readFile(String fileName) {
        String strReturn = null;
        byte[] arrByte = FileUtil.fileToBytes(fileName);
        if (arrByte != null) {
            ...
            if (file.exists()) {
                if (file.isFile()) {
                    ...
                    for (int i = 0; i < files.length; i++) {
                        FileUtil.deleteAllFile(files[i]);
                    }
                }
                if (file.exists()) {
                    file.delete();
    ...
    public static void main(String[] args) {
        ...
        //
        // byte[] arrByte = FileUtil.fileToBytes(fromFile);
